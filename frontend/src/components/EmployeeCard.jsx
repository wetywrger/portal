    ...
    <>
        ...
        <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full p-6 relative animate-scale-in" onClick={e => e.stopPropagation()}>
          ...
          <div className="flex flex-col md:flex-row gap-6 mb-6">
            <div className="relative group">
              <img src={employee.photo_url || '/placeholder.jpg'} alt="Фото" onClick={handleImageClick} className={`w-32 h-32 rounded-2xl object-cover border-4 border-warm-100 bg-slate-50 shadow-sm transition-all ${employee.photo_url ? 'cursor-pointer hover:opacity-90 hover:scale-[1.02]' : ''}`} />
              {employee.photo_url && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <InfoBlock label="Отдел" value={employee.department} />
      ...
      {showEnlarged && employee.photo_url && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 z-[60] animate-fade-in" onClick={() => setShowEnlarged(false)}>
          ...
          <img src={employee.photo_url} alt="Увеличенное фото" className="max-w-[90vw] max-h-[90vh] rounded-xl shadow-2xl object-contain cursor-default select-none" onClick={e => e.stopPropagation()} />
